...
class MY_Form_validation extends CI_Form_validation
{
	...
	/**
	 * Allow for spaces in alpha numeric check
	 *
	 * @access public
	 *
	...
	/**
	 * Validate AU mobile number
	 *
	 * @author Pham Long <zerokavn@gmail.com>
	 * @access public
	 *
	...
	{
		$normalized = preg_replace('/(\s+|-|\(|\))/', '', preg_replace('/0011\s?61/', '+61', $number));
		$pattern = "/^04[0-9]{8}$/";
		if (!(bool)preg_match($pattern, $normalized))
		{
			$this->CI->form_validation->set_message('validateMobile', lang('bf_form_phone'));
			return false;
		}
		return true;
	}//end validateMobile()
	...
	function validatePhone($number)
	{
		...
		if (!(bool)preg_match($pattern, $normalized))
		{
			$this->CI->form_validation->set_message('validatePhone', lang('bf_form_phone'));
			return false;
		}
		return true;
	}//end validatePhone()
	...
	/**
	 * Validate address with http://www.addressify.com.au
	 *
	 * @author Pham Long <zerokavn@gmail.com>
	 * @access public
	 *
	 ...
	 * @param $suburb
	 * @param $state
	 * @return boolean      Return true on OK, false on fail
	 */
	function address_check($postcode) {
		$postcode = (int)$postcode;
		$suburb = trim($this->CI->input->post('suburb'));
		$state = trim($this->CI->input->post('state'));
	
		if($postcode <= 0 || $suburb == '' || $state == '')
		{
			$this->CI->form_validation->set_message('address_check', lang('us_address_invalid'));
			return FALSE;
		}

		return true;
	
		if(!$this->checkDailyQuota(1000))
		{
			$this->CI->form_validation->set_message('address_check', lang('us_cannot_check_address'));
			return FALSE;
		}
	
		$address = strtoupper("$suburb,$state$postcode");
        $address = str_replace(" ", "", $address);  // remove any spaces for best match

		$suburbs = json_decode(file_get_contents("http://api.addressify.com.au/address/getSuburbsforPostcode?api_key=" . addressify_api_key . "&term=" . $postcode));
		
		// remove spaces for consistency
        $clean_suburbs = array();
        if (is_array($suburbs)) {
            foreach ($suburbs as $the_suburb) {
                $clean_suburbs[] = str_replace(" ", "", $the_suburb);
            }  
        }
        
		if(!in_array($address, $clean_suburbs))
		{
			$this->CI->form_validation->set_message('address_check', lang('us_address_invalid'));
			return FALSE;
		}
	...
	function checkDailyQuota($quota)
	{
		$used = file_get_contents("http://api.addressify.com.au/account/getDailyAPICallCount?api_key=" . addressify_api_key);
		if($used >= $quota)
		{
			// send alert mail
			...
			$data = array(
					'to'      => ALERT_MAIL_TO,
					'subject' => lang('us_api_limit_alert'),
					'message' => "API key: " . addressify_api_key,
			);
