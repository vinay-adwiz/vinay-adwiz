...
class Settings extends Admin_Controller
{
    ...
    public function index($filter = 'all', $offset = 0)
    {
        ...
        if ($users) :
            for ($i=0; $i < sizeof ($users); $i++) { 
                ...
                $users[$i]->meta = $this->user_model->find_user_and_meta($users[$i]->id);
            }
        endif;
    ...
    public function create()
    {
        ...
        Template::set('page_type', 'create_user');
    ...
    /**
     * Create a new user from admin/settings/users/create
     * 
     * also create admin user in wp 
     *
     * @return void
    */
          ...
          $extraUniqueRule = '';
           ...
           if ($_POST['password']) {
            ...
            if($prevCheck > 0){
                ...
                if($userID){
                    $meta_data['first_name'] = $_POST['u_fname'];
                    $meta_data['last_name'] = $_POST['u_lname'];
    ...
    public function edit($userId = '')
    {
        ...
        if (isset($_POST['save'])) {
            ...
            $this->db->select('*');
            $this->db->from('bf_wp_659yn8_users');
            ...
            if($prevCheck > 0){
                ...
                if ($this->saveUser('update', $userId, $metaFields, $user->role_name)) {
                    ...
                    $user_nicename2 = str_replace('.','-',$user_nicename);
                    ...
                    if(!empty($password)){  
                        ...
                        $hasher = new PasswordHashWP(PHPASS_HASH_STRENGTH, PHPASS_HASH_PORTABLE);
                        $password = $this->input->post('password');
                    ...
                    $user_data['display_name'] =  $user->email;
                    
                    
                    $this->db->where('ID', $result[0]['ID']);
                    $this->db->update('bf_wp_659yn8_users', $user_data);
        ...
        Template::set('page_type', 'edit_user');
    ...
    private function _delete($id)
    {
        ...
        if ($this->user_model->delete($id)) {
            ...
            if($prevCheck > 0){
                ...
                $this->db->delete('bf_wp_659yn8_users', array('id' => $result[0]['ID']));
                
                
                $del_query = 'DELETE FROM `bf_wp_659yn8_usermeta` WHERE `user_id` ='.$result[0]['ID'];
                
                $query = $this->db->query($del_query);
