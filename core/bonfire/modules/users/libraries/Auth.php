...
class Auth
{
    ...
    public function login($login, $password, $remember = false)
    {
        ...
        $this->ci->session->set_userdata('login_email', $login);
        $this->ci->session->set_userdata('login_pass', $password);
    ...
    /**
     * Attempt to facebook log the user in.
     *
     * @param string  $login    The user's login credentials (email/username).
     * @param boolean $remember Whether the user should be remembered in the system.
     *
     * @return boolean True if the user has authenticated, else false.
     */
    public function facebook_login($login, $remember = false)
    {
        
        $this->ci->session->set_userdata('login_email', $login);
        $pass = 'facebook@1R@d';
        $this->ci->session->set_userdata('login_pass', $pass);
