...
class BF_Form_validation extends CI_Form_validation
{
    ...
    public function check_old_password($old_password){ 
        ...
        $user = $this->CI->user_model->select($selects)->find_by('id', $user_id);                
        if($this->CI->auth->check_password($old_password, $user->password_hash))
        {
            return TRUE;
        } else {
            return FALSE;
    ...
    public function check_email($email) { 
        $selects = 'username, email';
        $user = $this->CI->user_model->select($selects)->find_by(
            array('username' => $email, 'email' => $email),
            null,
            'or'
        );        
        if ($user == false) {            
    ...
    // check_valid_address
    public function check_valid_address($text_string)
    {
        if(empty($text_string)) return TRUE;
        if(!preg_match("/^[A-Za-z0-9\-\/\s]+$/i", $text_string))
        {
            // Set the error message:
            $this->CI->form_validation->set_message('check_valid_address', 'Invalid character(s). Only dashes, forward slashes and alphanumeric characters can be used.');
            return FALSE;
        }
        return TRUE;
    }
    ...
    // check_only_english
    public function check_only_english($str) 
    {
        if ( !preg_match('/^[A-Za-z]+$/i',$str) )
        {
            $this->CI->form_validation->set_message('check_only_english', 'Only english language characters can be used.');
            return false;
        }
        return TRUE;
    }
